...
- 🌐 **OpenRouter provider endpoints**: `GET /openrouter/models` (full provider schema), `GET /privacy`, SSE `: keep-alive` every 10s during prefill/think, early-429 concurrency discipline, bearer-key auth — live behind a Cloudflare tunnel on dedicated A100 silicon. Provider **schema finalized for listing** (2026-07-13): `supported_features:["reasoning"]`, `quantization:"int4"`, model id `openhubresearch/olmo3-32b` (AWQ 4-bit weights) — all fields overridable via `ATLAS_OR_*` env (incl. `ATLAS_OR_FEATURES`)
